...
class ImageCaptcha(_Captcha):
    ...
    def create_captcha_image(self, chars, color, background, offset):
        ...
        return image, offset_list

    def generate_image(self, chars, offset=0):
        """Generate the image of the given characters.

        :param chars: text to be generated.
        """
        background = random_color(238, 255)
        color = random_color(3, 200, random.randint(220, 255))
        im, offset_list = self.create_captcha_image(chars, color, background, offset)
        self.create_noise_dots(im, color)
        self.create_noise_curve(im, color)
        im = im.filter(ImageFilter.SMOOTH)
        return im, offset_list
